...
Sign in to a personal OpenSRE account with GitHub, inspect the local login, or sign out. Same as `opensre account`. The shell starts only for an `active` account state. `/account logout` revokes the saved session, invalidates the hosted-model route, and closes the shell before another turn. If `OPENSRE_ACCOUNT_TOKEN` is set in the environment, it overrides the token saved by login until you unset it.
...
Run the first-use webapp account flow. It creates or signs in to an OpenSRE account with GitHub and activates the account-hosted model. Setup requires exclusive stdin because browser authorization prompts on the real TTY.
