...
<Steps>
  ...
  <Step title="Setup">
    Run first-run setup to create or sign in to your OpenSRE account with GitHub:

    ```bash
    opensre setup
    ```

    The webapp login connects GitHub and activates the OpenSRE-hosted model. When setup finishes, OpenSRE validates the new session and opens the interactive shell. Add integrations later with `opensre integrations setup <service>` when you want OpenSRE to query a specific tool.
