...
Inspect repository structure and layering, save the evidence, and complete the
...
- Order: repository scan before heuristics (≤3 shell_run; use discovered roots,
...
This skill hides its shell output (`quiet=true`), so the pinned plan overlay
is the user's progress signal.

- Call `update_plan` **once at the start** of the run with the nine workflow
  steps below (pending). Mark the first step `in_progress`.
...
Call `architecture_clone_repo(owner, repo, ref?)`.
The returned `workspace_root` is `<system-temp>/opensre/workspace`.
(If already in the target checkout and no owner/repo named, skip clone and
use cwd as workspace_root; then skip cleanup.)

### 2. Scan the repository

Orient on the repo before heuristics (at most three `shell_run` calls,
...
Call `shell_run(command=..., quiet=true)` for this pass.

Using repository scan layout/docs, discover the repo's stated layer/module
...
Using repository scan layout, discover the repo's package/module placement
...
sources. Prefer primary source roots discovered in repository scan; skip only
...
### 6. Check shims and facades

Call `shell_run(command=..., quiet=true)` for this pass.

Lightweight heuristic for compatibility / re-export / facade modules across
languages, scoped to roots from repository scan. Distinguish deliberate public
...
Call `architecture_cleanup_repo()` as the next tool after step 6.
(Skip when step 1 used cwd and did not clone.)

### 8. Save observations

Call `architecture_save_observations(repo_name, observations)` after cleanup
(or after the shim pass when cleanup was skipped), before the final no-tool
report. Ending the turn before this call skips it.
...
In the final reply, without further tool calls, fill
`core/agent_harness/prompts/skills/architecture_audit/architecture_audit_report.md`
from passes 3–6 grounded in repository scan context. Summarize; never paste huge
